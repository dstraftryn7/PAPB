...
class TaskPage extends StatefulWidget {
  ...
  const TaskPage({Key? key, required this.boardTitle, required this.boardId})
      : super(key: key);
...
class _TaskPageState extends State<TaskPage> {
  ...
  Future<Map<String, dynamic>?> getData() async {
    ...
    if (userdataget != null) {
      final myUserData = jsonDecode(userdataget)['user'];
      myid = myUserData['id'];
    }
    return null;
  }
    ...
    return FutureBuilder(
      ...
      builder: (context, _) => Scaffold(
        appBar: AppBar(
          ...
          elevation: 0,
          iconTheme:
              const IconThemeData(color: Color.fromARGB(255, 68, 56, 80)),
          title: Row(children: [
            Image.asset(
              'assets/logo.png',
              width: 35,
...
class TaskFormDialog extends StatefulWidget {
  ...
  final int getUserId;
  final int getBoardId;

  const TaskFormDialog(
      {Key? key,
      required this.getUserId,
      required this.getBoardId,
          ...
          onPressed: () {
            if (title.isNotEmpty) {
              ...
              BoardController.addTask(
                  ...
                  datetime: newTask.combinedDateTime.toString(),
                  boards_id: widget.getBoardId,
                  user_id: widget.getUserId.toInt(),
                  context: context);
